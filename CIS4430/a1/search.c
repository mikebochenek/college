...
node * search (void * node_ptr, char * value)
{
	...
	else if ( ((interior_node *) node_ptr)->is_leaf == FALSE)
	{
		...
		{
			return search ( current->memory_pointer[0], value);
		}
		...
		for (i = 0; i < N_PARAMETER && current->key[i][0] != '\0'; i++)
		{
			...
			/* if (compare <= 0) */
			{
				return search ( current->memory_pointer[i], value);
			}
		}
...
node * search_range (void * node_ptr, int * size, char * value, char *upper_value)
{
	...
	else if ( ((interior_node *) node_ptr)->is_leaf == FALSE)
	{
		...
		if (strcmp (value, current->key[0]) < 0)
		{
			return search_range ( current->memory_pointer[0], size, value, upper_value);
		}
		...
		for (i = 0; i < N_PARAMETER && current->key[i][0] != '\0'; i++)
		{
			...
			/* if (compare <= 0) */
			{
				return search_range ( current->memory_pointer[i], size, value, upper_value);
			}
		}
